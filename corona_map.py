...
def make_subway_layer(Map):
  ...
  for i in range(0,40):
    subway_layer.add_child(folium.CircleMarker(
              location=[ line1['위도'][i], line1['경도'][i] ],
              radius=3, color="#F6471B", 
              fill_color='#FFFFFF', fill=True,opacity=opacity))

    try:
      p1=[ line1['위도'][i]   ,  line1['경도'][i] ]
      p2=[ line1['위도'][i+1] ,  line1['경도'][i+1] ]
  ...
  for i in range(41,83):
    subway_layer.add_child(folium.CircleMarker(
              location=[ line2['위도'][i], line2['경도'][i] ], 
              radius=3, color='#47DD77', 
              fill_color='white', fill=True, opacity=opacity))
    busan_map.add_child(subway_layer)

    try:
      p1=[ line2['위도'][i]   , line2['경도'][i] ]
      p2=[ line2['위도'][i+1] , line2['경도'][i+1] ]
  ...
  for i in range(84,100):
    subway_layer.add_child(folium.CircleMarker(
              location=[ line3['위도'][i], line3['경도'][i] ], radius=3, color='#ECBC26', 
              fill_color='white', fill=True, opacity=opacity))

    try:
      p1=[ line3['위도'][i]   , line3['경도'][i] ]
      p2=[ line3['위도'][i+1] , line3['경도'][i+1] ]
  ...
  for i in range(101,114):
    subway_layer.add_child(folium.CircleMarker(
              location=[ line4['위도'][i],line4['경도'][i] ], 
              radius=3, color='#2683EC', 
              fill_color='white', fill=True, opacity=opacity))

    try:
      p1=[ line4['위도'][i]   ,  line4['경도'][i] ]
      p2=[ line4['위도'][i+1] ,  line4['경도'][i+1] ]
  ...
  for i in range(115,135):
    subway_layer.add_child(folium.CircleMarker(
              location=[ line_kimhae['위도'][i], line_kimhae['경도'][i] ], 
              radius=3, color='#3B1ACC', 
              fill_color='white', fill=True, opacity=opacity))
    busan_map.add_child(subway_layer)
    
    try:
      p1=[ line_kimhae['위도'][i]   ,  line_kimhae['경도'][i] ]
      p2=[ line_kimhae['위도'][i+1] ,  line_kimhae['경도'][i+1] ]
  ...
  for i in range(136,149):
    subway_layer.add_child(folium.CircleMarker(
              location=[ line_donghae['위도'][i], line_donghae['경도'][i] ], 
              radius=3, color='#B42BA7', 
              fill_color='white', fill=True, opacity=opacity))

    try:
      p1=[ line_donghae['위도'][i]   ,  line_donghae['경도'][i] ]
      p2=[ line_donghae['위도'][i+1] ,  line_donghae['경도'][i+1] ]
